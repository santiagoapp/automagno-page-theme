...
if ( bp_is_group_creation_step( 'group-avatar' ) ) :
  $form_class = 'standard-form x-create-group-form x-avatar-upload-form cf';
else :
  $form_class = 'standard-form x-create-group-form cf';
endif;
...
<div id="buddypress">
	...
	<?php do_action( 'bp_before_create_group_content_template' ); ?>
		...
		<div class="x-item-list-tabs-nav item-list-tabs no-ajax" id="group-create-tabs" role="navigation">
			<ul>
	...
	<form action="<?php bp_group_creation_form_action(); ?>" method="post" id="create-group-form" class="<?php echo $form_class; ?>" enctype="multipart/form-data">
		...
		<div class="item-body" id="group-create-body">
			...
			<?php if ( bp_is_group_creation_step( 'group-settings' ) ) : ?>
				...
				<?php if ( bp_is_active( 'forums' ) ) : ?>
					...
					<h4><?php _e( 'Group Forums', '__x__' ); ?></h4>

					<?php if ( bp_forums_is_installed_correctly() ) : ?>

						<p><?php _e( 'Should this group have a forum?', '__x__' ); ?></p>

						<div class="checkbox">
							<label><input type="checkbox" name="group-show-forum" id="group-show-forum" value="1"<?php checked( bp_get_new_group_enable_forum(), true, true ); ?> /> <?php _e( 'Enable discussion forum', '__x__' ); ?></label>
						</div>
					<?php elseif ( is_super_admin() ) : ?>

						<p><?php printf( __( '<strong>Attention Site Admin:</strong> Group forums require the <a href="%s">correct setup and configuration</a> of a bbPress installation.', '__x__' ), bp_core_do_network_admin() ? network_admin_url( 'settings.php?page=bb-forums-setup' ) :  admin_url( 'admin.php?page=bb-forums-setup' ) ); ?></p>
			...
			<?php if ( bp_is_group_creation_step( 'group-avatar' ) ) : ?>
				...
				<?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
					...
					<div class="main-column">
						...
						<p class="skip"><?php _e( 'To skip the avatar upload process, hit the "Next" button.', '__x__' ); ?></p>
					</div><!-- .main-column -->
				...
				<?php if ( 'crop-image' == bp_get_avatar_admin_step() ) : ?>
					...
					<h4><?php _e( 'Crop Group Avatar', '__x__' ); ?></h4>
			...
			<?php if ( bp_is_group_creation_step( 'group-invites' ) ) : ?>
				...
				<?php else : ?>
					...
					<div id="message" class="info">
						<p><?php _e( 'Once you have built up friend connections you will be able to invite others to your group.', '__x__' ); ?></p>
					</div>
			...
			<?php if ( 'crop-image' != bp_get_avatar_admin_step() ) : ?>
				...
				<div class="submit" id="previous-next">
					...
					<?php if ( !bp_is_first_group_creation_step() ) : ?>
						...
						<input type="button" value="<?php esc_attr_e( 'Back', '__x__' ); ?>" id="group-creation-previous" name="previous" onclick="location.href='<?php bp_group_creation_previous_link(); ?>'" />
					...
					<?php if ( !bp_is_last_group_creation_step() && !bp_is_first_group_creation_step() ) : ?>
						...
						<input type="submit" value="<?php esc_attr_e( 'Next', '__x__' ); ?>" id="group-creation-next" name="save" />
					...
					<?php if ( bp_is_first_group_creation_step() ) : ?>
						...
						<input type="submit" value="<?php esc_attr_e( 'Create', '__x__' ); ?>" id="group-creation-create" name="save" />
					...
					<?php if ( bp_is_last_group_creation_step() ) : ?>
						...
						<input type="submit" value="<?php esc_attr_e( 'Finish', '__x__' ); ?>" id="group-creation-finish" name="save" />
